...
// handle also requires the service
discovery.require('service.queue', 'service.data.redis', 'service.register');

discovery.handle('service.queue', function(service) {
  rabbit = mq.createConnection(service.config);
  rabbit.on('ready', discovery.after('rabbit'));
});

discovery.handle('service.data.redis', function(service) {
  redis = db.createClient(service.config.port, service.config.host);
  if (service.config.auth)
    redis.auth(service.config.auth, discovery.after('redis'));
});

// only executes after queue discovered and rabbitmq connected
discovery.handle('rabbit', 'service.register', function(service) {
  var done = _.after(2, discovery.after('rabbit.configure'));
  ...
  var exchange = rabbit.exchange(service.config.output, {autoDelete: false, type: 'fanout'}, next);
  creation = rabbit.queue({exclusive: true}, next);
  // for outgoing registrations
  register = rabbit.exchange(service.config.input, {durable: true, confirm: true, autoDelete: false, type: 'fanout'}, done);
});
