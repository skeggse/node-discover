...
var wrapError = function(ignoreBad, error, callback) {
  if (ignoreBad)
    callback();
  else
...
UnwrapStream.prototype._transform = function(chunk, encoding, callback) {
  if (chunk.length <= 36)
    return wrapError(this.ignoreBad, 'block is too small', callback);
  if (chunk.toString(null, 0, 4) !== 'WRAP')
    return wrapError(this.ignoreBad, 'block not a wrapped buffer', callback);
  var header = {};
  ...
  if (chunk.length < header.blockLength)
    return wrapError(this.ignoreBad, 'block incomplete', callback);
...
var JSONEncodeStream = function JSONEncodeStream(options) {
  ...
  this.ignoreBad = (options && options.ignoreBad) || jsonDefaults.ignoreBad;
};
...
JSONEncodeStream.prototype._transform = function(obj, encoding, callback) {
  ...
  try {
    ...
    return callback(this.ignoreBad ? null : e);
  }
  // workaround
